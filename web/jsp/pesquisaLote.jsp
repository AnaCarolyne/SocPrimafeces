...
<html>
    ...
    <body>
        <h1>Pesquisa de Lote ${operacao}</h1>
       <form action="ManterLoteController?acao=prepararIncluir" method="post" name="frmManterLote" onsubmit="return validarFormulario(this)">
        ...
        <table border=1>
            ...
            <c:forEach items="${lotes}" var="lote">
                <tr>
                    ...
                    <td><c:out value="${lote.dataComeco}"/></td>
                    <td><c:out value="${lote.dataFim}"/></td>
                    <td><c:out value="${lote.valor}"/></td>
                    <td><a href="ManterLoteController?acao=prepararEditar&idLote=<c:out value="${lote.idLote}" />"> Editar</a></td>
                    <td><a href="ManterLoteController?acao=prepararExcluir&idLote=<c:out value="${lote.idLote}" />">Excluir</a></td>
                </tr>      
            </c:forEach>
        </table>
