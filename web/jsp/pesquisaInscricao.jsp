    ...
    <head>
       ...
       <style type="text/css"> 
        ...
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
         <center>
        <title>Pesquisa de Inscricao</title>
    </head>
    <body>
        <h1 >Pesquisa de Inscricao ${operacao}</h1>
    <form action="ManterInscricaoController?acao=prepararIncluir" method="post" name="frmManterInscricao" onsubmit="return validarFormulario(this)">
        <table border=1>
            <tr>
                <th>Codigo Inscricao</th>
                <th>Boleto</th>
            ...
            <c:forEach items="${inscricaos}" var="inscricao">
                <tr>
                    <td><c:out value="${inscricao.idInscricao}"/></td>
                    <td><c:out value="${inscricao.boleto}"/></td>
                     <td><c:out value="${inscricao.numeroDePeito}"/></td>
                     
                    
                    <td><a href="ManterInscricaoController?acao=prepararEditar&idInscricao=<c:out value="${inscricao.idInscricao}" />"> Editar </a></td>
                    <td><a href="ManterInscricaoController?acao=prepararExcluir&idInscricao=<c:out value="${inscricao.idInscricao}" />">Excluir</a></td>
                </tr>      
            </c:forEach>
        </table>
