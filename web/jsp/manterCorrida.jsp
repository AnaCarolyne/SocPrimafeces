...
<html>
    ...
    <SCRIPT language="JavaScript">
        ...
        {
            var caracteresValidos = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ çãâá";
            var ehString = true;
            var umCaracter;
        ...
        function validarFormulario(form) {
            ...
            if (!campoNumerico(form.idCorrida.value)) {
                mensagem = mensagem + "O ID deve ser numerico\n";
            }
            if (form.nome.value == "") {
                mensagem = mensagem + "Informe o Nome da Corrida\n";
            }
            if (!campoString(form.nome.value)) {
                mensagem = mensagem + "O Nome deve ser texto\n";
            }
            if (form.maximoAtletas.value == "") {
                mensagem = mensagem + "Informe o Maximo de Atletas \n";
            }
            if (!campoNumerico(form.maximoAtletas.value)) {
                mensagem = mensagem + "O Maximo de Atletas deve ser numerico\n";
            }
            if (form.estado.value == "") {
                mensagem = mensagem + "Informe o Estado da Corrida\n";
            }
            if (!campoString(form.estado.value)) {
                mensagem = mensagem + "O Estado deve ser texto\n";
            }
            if (form.idOrganizador.value == "") {
                mensagem = mensagem + "Informe o Id do Organizador\n";
            }
            if (form.idSuprimento.value == "") {
                mensagem = mensagem + "Informe o Id do Suprimento\n";
            }
    ...
    <body>
        <div aligan="center">
            ...
            <form id="testForm" action="ManterCorridaController?acao=confirmar${operacao}" method="post" onsubmit="return validarFormulario(this)">

                <table>
                    <tr>
                        <td>Código da Corrida</td> 
                        <td><input type="text" name="idCorrida" value="${corrida.idCorrida}" <c:if test="${operacao != 'Incluir'}"> readonly</c:if>></td>
                        </tr>
                        <tr>
                            <td>Nome da Corrida:</td> 
                            <td><input type="text" name="nomeCorrida" value="${corrida.nomeCorrida}" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>></td>
                        </tr>
                        <tr>
                            <td>Data da Corrida:</td> 
                            <td><input type="text" name="data" value="${corrida.data}" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>></td>
                        </tr>
                        <tr>
                            <td> Horario da Corrida:</td> 
                            <td><input type="text" name="horario" value="${corrida.horario}" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>></td>
                        </tr>
                        <tr> 
                            <td>Maximo de Atletas:</td> 
                            <td><input type="text" name="maximoAtletas" value="${corrida.maximoAtletas}" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>></td>
                        </tr>
                        <tr> 
                            <td>Estado:</td> 
                            <td><input type="text" name="estado" value="${corrida.estado}" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>></td>
                        </tr>
                        <tr>
                            <td>Organizador:</td> 
                            <td>
                                <select name="idOrganizador" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>>
                                <option value="0" <c:if test="${corrida.organizadoridOrganizador == null}"> selected</c:if>> </option>  
                                <c:forEach items="${organizadores}" var="organizador">
                                    <option value="${organizador.idOrganizador}" <c:if test="${corrida.organizadoridOrganizador.idOrganizador == organizador.idOrganizador}"> selected</c:if>>${organizador.cpf}</option>  
                                </c:forEach>
                            </select>
                        </td>
                    </tr>                               
