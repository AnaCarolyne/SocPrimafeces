...
<html>
    <head>
        ...
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
         <center>
        <title>Pesquisa de Chip  ${operacao}</title>
         <form action="ManterChipController?acao=prepararIncluir" method="post" method="post" name="frmManterChip" onsubmit="return validarFormulario(this)">

         </head>
    <body>
        <h1>Pesquisa de Chip</h1>
        ...
        <table border=2>
            ...
            <c:forEach items="${chips}" var="chip">
                <tr>
                    <td><c:out value="${chip.idChip}"/></td>
                    <td><c:out value="${chip.numeroDePeito}"/></td>
                    <td><c:out value="${chip.tempo}"/></td>
                    <td><a href="ManterChipController?acao=prepararEditar&idChip=<c:out value="${chip.idChip}" />">Editar</a></td>
                    <td><a href="ManterChipController?acao=prepararExcluir&idChip=<c:out value="${chip.idChip}" />"> Excluir</a></td>
                </tr>      
            </c:forEach>
        </table>
