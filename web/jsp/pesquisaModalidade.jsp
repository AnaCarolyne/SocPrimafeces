...
<html>
    ...
    <body>
        <h1>Pesquisa de Modalidade ${operacao}</h1>
        <form action="ManterModalidadeController?acao=prepararIncluir" method="post" method="post" >

        <table border=1>
            <tr>
         
                <th>Codigo Modalidade</th>
                <th>Sexo</th>
                <th>Percurso</th>
                <th>Hora da Largada</th>
                <th>Tipo de Prova</th>
                <th>Quilometragem</th>
            ...
            <c:forEach items="${modalidades}" var="modalidade">
                <tr>
                    <td><c:out value="${modalidade.idModalidade}"/></td>
                    <td><c:out value="${modalidade.sexo}"/></td>
                    <td><c:out value="${modalidade.percurso}"/></td>
                    <td><c:out value="${modalidade.horaLargada}"/></td>
                    <td><c:out value="${modalidade.tipoProva}"/></td>
                    <td><c:out value="${modalidade.quilometragem}"/></td>
                    <td><a href="ManterModalidadeController?acao=prepararEditar&idModalidade=<c:out value="${modalidade.idModalidade}" />">Editar</a></td>
                    <td><a href="ManterModalidadeController?acao=prepararExcluir&idModalidade=<c:out value="${modalidade.idModalidade}" />"> Excluir</a></td>
                </tr>      
            </c:forEach>
        </table>
        <br>
