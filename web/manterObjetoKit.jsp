...
<html>
      <SCRIPT language="JavaScript">
            ...
            function validarFormulario(form) { 
                var mensagem;
                mensagem = "";
                if (form.idObjetoKit.value == ""){
                    mensagem = mensagem + "Informe o Id\n";
                }
                if (!campoNumerico(form.idObjetoKit.value)){
   ...
   <head>
    ...
    <body>
        <div aligan="center">
                <h1>Manter ObjetoKit - ${operacao}</h1>
                 <form id="testForm" action="ManterObjetoKitController?acao=confirmar${operacao}" method="post" onsubmit=" return validarFormulario(this)">
                    <table>
                        <tr>
                            <td>Código doObjetoKit</td> 
                            <td><input type="text" name="idObjetoKit" value="${objetokit.idObjetoKit}" <c:if test="${operacao != 'Incluir'}"> readonly</c:if>></td>
                            </tr>
                            <tr>
                                <td>Nome:</td> 
                                <td><input type="text" name="nomeObjetoKit" value="${objetokit.nomeObjetoKit}" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>></td>
                            </tr>
                         
                            <tr>
                                <td> Descricao:</td> 
                                <td><input type="text" name="descricao" value="${objetokit.descricao}" <c:if test="${operacao == 'Excluir'}"> readonly</c:if>></td>
                            </tr>
                            <tr> 
                                <td>Quantidade:</td> 
                                  <td align="left"><input name="quantidade" type="number" value="${objetokit.quantidade}"  min="1" size="100"> </td>
          <tr>
