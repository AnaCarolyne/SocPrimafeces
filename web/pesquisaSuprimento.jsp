...
<html>
    <head>
        ...
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
         <center>
        <title>Pesquisa de Suprimento</title>
    </head>
    <body>
        <h1>Pesquisa de Suprimento  ${operacao}</h1>
        
        <table border=1>
            <tr>
                <th>Codigo Suprimento</th>
            ...
            <c:forEach items="${suprimentos}" var="suprimento">
                <tr>
                    <td><c:out value="${suprimento.idSuprimento}"/></td>
                    <td><c:out value="${suprimento.nomeSuprimento}"/></td>
                    <td><c:out value="${suprimento.descricao}"/></td>
                    <td><c:out value="${suprimento.quantidade}"/></td>
                     <td><c:out value="${suprimento.marca}"/></td>
                     <td><c:out value="${suprimento.corridaidCorrida.nomeCorrida}"/></td>
                    <td><a href="ManterSuprimentoController?acao=prepararEditar&idSuprimento=<c:out value="${suprimento.idSuprimento}" />">Editar </a></td>
                    <td><a href="ManterSuprimentoController?acao=prepararExcluir&idSuprimento=<c:out value="${suprimento.idSuprimento}" />">Excluir </a></td>
                </tr>      
            </c:forEach>
        </table>
        <form action="ManterSuprimentoController?acao=prepararIncluir" method="post">
             <br>
            <br>
             <tr>
